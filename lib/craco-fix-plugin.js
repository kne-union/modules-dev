...
const isolateWebpackCacheIfNodeModulesSymlink = (webpackConfig) => {
    if (!env.isNodeModulesSymlink || !webpackConfig.cache || webpackConfig.cache.type !== 'filesystem') {
        return;
    }
    webpackConfig.cache = Object.assign({}, webpackConfig.cache, {
        cacheDirectory: path.resolve(env.modulesDevDir, 'webpack-cache')
    });
};
...
module.exports = {
    overrideWebpackConfig({webpackConfig, context}) {
        ...
        isolateWebpackCacheIfNodeModulesSymlink(webpackConfig);
