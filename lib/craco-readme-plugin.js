...
const ReadmePlugin = {
    overrideDevServerConfig: ({devServerConfig}) => {
        ...
        const {isFound, match} = getLoader(webpackConfig, loaderByName("css-loader"));
        if (isFound) {
            match.loader.options = merge({}, match.loader.options, {
                modules: {
                    localIdentHashSalt: `${componentsName}:${componentsVersion || '1.0.0'}`
                }
            });
        }
